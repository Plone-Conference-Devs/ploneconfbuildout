...
def html(title, body): 
   s=''
   #try to load the html from an external file and insert page into it.
   if os.path.exists('pwyky.html'): 
      #open html
      s = open('pwyky.html').read()
      s = s.replace('PWYKY-PROFILE-GOES-HERE', profile)
      s = s.replace('PWYKY-TITLE-GOES-HERE', title)
      s = s.replace('PWYKY-CONTENT-GOES-HERE', body)
   #else revert to original way:
   else:
